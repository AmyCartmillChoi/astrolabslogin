...
class signup extends Component {
  ...
  submitForm = async(e) => {
    ...
    try {
      let response = await signupRequest({email: this.state.email, password: this.state.password, name: this.state.name});
      saveUser(response);
      this.setState({
        loggedin: false,
        areregistered: true
  ...
  render(){
    return (<Layout>
      ...
      <div className="form-group">
        ...
        <small id="nameHelp" className="form-text text-muted">So...Whats ya name...</small>
      </div>
      <div className="form-group">
